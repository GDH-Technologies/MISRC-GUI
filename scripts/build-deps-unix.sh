...
if [[ -f "$stamp_file" ]] && \
   { [[ -f "$DEPS_PREFIX/lib/pkgconfig/hsdaoh.pc" ]] || [[ -f "$DEPS_PREFIX/lib64/pkgconfig/hsdaoh.pc" ]]; }; then
...
# -DCMAKE_INSTALL_LIBDIR=lib: on Fedora and other GNUInstallDirs multilib
# distros CMake installs to lib64/, but hsdaoh's generated .pc hardcodes
