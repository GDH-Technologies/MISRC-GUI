...
/* A per-session read credential, from the kernel's CSPRNG.
 *
 * Never rand(): it is seeded from the clock, so a password generated by it is
 * guessable from roughly when the stream started -- which anyone who can see
 * the stream exists already knows.
 *
 * 16 characters from a 32-symbol alphabet is 80 bits. The alphabet drops 0/O
 * and 1/I/L because this is a password people read aloud across a room, and
 * the dashes are there for the same reason. */
static bool rs_make_password(char *out, size_t cap)
{
    ...
    if (got < sizeof(raw)) {
        ...
        while (got < sizeof(raw)) {
            ...
            if (n < 0 && errno == EINTR) continue;
            close(fd);
            return false;                       /* refuse rather than weaken */
        }
        close(fd);
    }
...
int gui_rtsp_stream_start(const gui_rtsp_stream_opts_t *opts, char *err, size_t err_cap)
{
    ...
    /* The credential has to exist before the server is started, because it
     * lives in the config file mediamtx reads at startup. */
    gui_mediamtx_config_t ports = opts->ports;
    ports.read_password[0] = '\0';
    char password[40] = {0};
    if (opts->want_password) {
        if (!rs_make_password(password, sizeof(password))) {
            snprintf(err, err_cap,
                     "could not draw a stream password from the system RNG");
            return -1;
        }
        snprintf(ports.read_password, sizeof(ports.read_password), "%s", password);
    }
     ...
     * A changed password is the one thing that does force a restart -- see
     * gui_mediamtx_start(). */
    if (gui_mediamtx_start(&ports, err, err_cap) != 0) {
        return -1;
    }

    memset(&rs.status, 0, sizeof(rs.status));
    snprintf(rs.status.password, sizeof(rs.status.password), "%s", password);
    ...
    rs.opts = *opts;
    rs.opts.ports = ports;   /* including the credential actually in force */
